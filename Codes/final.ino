...
#define NEAR 30         //공격수 공이 가까이 있을 때, 공을 끼고 너무 크게 돌면 이값을 키운다.
#define VERYNEAR 160    //공격수 공이 매우 가까이 있을 때, 공을 끼고 너무 크게 돌면 이값을 키운다.
...
void ball_get(int dir, int power)
{
  switch(dir)
  {
    case 0:
...
void ball_verynear(int dir, int power)
{
  switch(dir)
  {
    case 0:
...
void PROGRAM1(void)//공격 
{
  ...
  while(1)
  {
   if(LineDetected)    //경계라인이 감지되었으면 탈출 함.
    ...
    else if(max_ir >= 200 && ball_dir == 0)
    {
     blocks = pixy.getBlocks();
     if(CameraExist)
      {
       if(pixy.blocks[0].x <= 40 )        {move(100,70,-100); delay(3);}  //매우 크게 좌회전
       else if(pixy.blocks[0].x > 40 && pixy.blocks[0].x <= 75)   {move(100,50,-100); delay(3);}  //크게 좌회전
       else if(pixy.blocks[0].x > 75 && pixy.blocks[0].x <= 150)   {move(100,45,-100); delay(3);}  //좌회전
       else if(pixy.blocks[0].x >= 175 && pixy.blocks[0].x < 250)        {move(100,-45,-100); delay(3);}  //우회전
       else if(pixy.blocks[0].x >= 250 && pixy.blocks[0].x < 285)        {move(100,-50,-100); delay(3);}  //크게 우회전
       else if(pixy.blocks[0].x >= 285)        {move(100,-70,-100); delay(3);}  //매우 크게 우회전
       else move(100,0,-100);   {delay(6);}
      }
     else
      {
       move(100,0,-100);
        ...
        else if (max_ir >= 0 && max_ir < NEAR) 
         {
          ball_get(ball_dir, 100);
         }
        else if (VERYNEAR > max_ir && max_ir >= NEAR)
         {
          ball_near(ball_dir,90);
         }
        else if (max_ir >= VERYNEAR)
         {
          ball_verynear(ball_dir,80);       
         }
   }
}
