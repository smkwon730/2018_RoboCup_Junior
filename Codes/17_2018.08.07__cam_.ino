...
#define SUPERNEAR 180   //공격수 공이 매애애애애우 가까이 있을 때
...
void ball_supernear(int dir, int power)
{
  switch(dir)
  {
    case 0:
...
void PROGRAM1(void)//공격 
{
  ...
  while(1)
  {
   if(LineDetected)    //경계라인이 감지되었으면 탈출
     {
      move(0,0,0);
      delay(100);
   ...
   else if(ball_dir >= 12 )
   {
   ...
   else if (ball_dir == 0)
    {
      ...
      if(CameraExist)
      {
         if(pixy.blocks[0].x > 175 && pixy.blocks[0].x < 220)        move(80,-50,-80 );     // 만약 골대가 오른쪽에 있으면 우회전
         else if(pixy.blocks[0].x < 145 && pixy.blocks[0].x > 100)   move(80,50,-80 );      // 만약 골대가 왼쪽에 있으면 좌회전
         ...
         else if (SUPERNEAR > max_ir && max_ir >= VERYNEAR)
         {
         ball_verynear(ball_dir, 60);       
         }
         else if (max_ir >= SUPERNEAR)
         {
         ball_supernear(ball_dir, 50);
         }
         else if(ball_dir == 0 && max_ir >= 190)
         {
         move(100,0,-100);
         }
    }
