...
#define VERYNEAR 100    //공격수 공 근처의 기준값,  공을 끼고 너무 크게 돌면 이값을 키운다.
   ...
   if (ball_dir == 0)
    {
      if(CameraExist)
      {
         ...
         if(blocks)
         {
         if(pixy.blocks[0].x > 175)        move(50,-20,-50 );     // 만약 골대가 오른쪽에 있으면 우회전
         else if(pixy.blocks[0].x < 145)   move(50,20,-50 );      // 만약 골대가 왼쪽에 있으면 좌회전
         else if(pixy.blocks[0].x == 165)  move(80,0,-80 );       // 골대가 중앙에 있으면 직진
         }
      }
    }
         else if (max_ir < NEAR)  
         {
         dir_move(ball_dir,60);
         }  
         else if (VERYNEAR > max_ir && max_ir >= NEAR)
         {
         ball_near(ball_dir,60);
         }
