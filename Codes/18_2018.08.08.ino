...
#define NEAR 40         //공격수 공이 가까이 있을 때, 공을 끼고 너무 크게 돌면 이값을 키운다.
#define VERYNEAR 100    //공격수 공이 매우 가까이 있을 때
  ...
  while(1)
  {
   if(LineDetected)    //경계라인이 감지되었으면 탈출 함.
    ...
    else if(max_ir >= 200 && ball_dir == 0)
    {
     ...
     if(CameraExist)
      {
       if(pixy.blocks[0].x <= 80 )        {move(100,55,-100); delay(3);}  //크게 좌회전
       else if(pixy.blocks[0].x >= 90 && pixy.blocks[0].x <= 140)   {move(100,45,-100); delay(3);}  //좌회전
       else if(pixy.blocks[0].x >= 190 && pixy.blocks[0].x <= 240)        {move(100,-45,-100); delay(3);}  //우회전
       else if(pixy.blocks[0].x >= 250 )        {move(100,-55,-100); delay(3);}  //크게 우회전
